...
from chesslab.engines.init_engines import get_distilled_maia, get_maia_range
...
if __name__ == "__main__":
    logger.info("Starting distillate maia script")
    folder = Path(__file__).parent / "results/distillate_maia"
    ...
    with get_session() as session:
        ...
        players = [get_distilled_maia(session=session, ratio=ratio) for ratio in ratios]
        ...
        with Evaluator() as evaluator:
            ...
            for ax, range_analysis, ratio in zip(ax_list, ranges_analysis, ratios):
                ...
                ax.set_title(f"Maia {int(ratio * 100)}% distillated")
