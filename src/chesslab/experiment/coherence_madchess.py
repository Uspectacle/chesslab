...
from chesslab.engines.init_engines import get_madchess_range
...
if __name__ == "__main__":
    logger.info("Starting coherence madchess script")
    folder = Path(__file__).parent / "results/coherence_madchess"
    ...
    with get_session() as session:
        players = get_madchess_range(session=session, num_step=6)

        opponents = get_madchess_range(session=session)
        ...
        with Evaluator() as evaluator:
            ...
            for ax, range_analysis in zip(ax_list, ranges_analysis):
                ...
                ax.set_title(f"MadChess {range_analysis.player.expected_elo} ELO")
                logger.info(f"MadChess {range_analysis.player.expected_elo} plotted")
