...
if __name__ == "__main__":
    logger.info("Std_dev variation script")
    folder = Path(__file__).parent / "results/std_dev_variation"
    ...
    with get_session() as session:
        ...
        players = [
            get_voting_player(crowd_std_dev=0, **options),
            get_voting_player(crowd_std_dev=100, **options),
            get_voting_player(crowd_std_dev=200, **options),
            get_voting_player(crowd_std_dev=400, **options),
            get_voting_player(crowd_std_dev=800, **options),
        ]
        names = [
            f"Elo spread (std dev) = {player.options.get('Crowd_std_dev')}"
