...
from chesslab.storage.game_tools import get_board
from chesslab.storage.schema import Game

logger = structlog.get_logger()


def export_game_to_pgn(game: Game) -> chess.pgn.Game:
    # Create PGN game
    board = get_board(game)
    pgn_game = chess.pgn.Game.from_board(board=board)

    # Set headers
    pgn_game.headers["Event"] = game.game_metadata.get("event", "ChessLab")
    pgn_game.headers["Site"] = game.game_metadata.get(
        "site", "https://github.com/Uspectacle/chesslab"
    )
    pgn_game.headers["Date"] = game.started_at.strftime("%Y.%m.%d")
    pgn_game.headers["Round"] = str(game.game_metadata.get("round", "-"))
    pgn_game.headers["White"] = (
        "{self.game.white_player.engine_type} (ID: {self.game.white_player.id})"
    )
    pgn_game.headers["Black"] = (
        "{self.game.balck_player.engine_type} (ID: {self.game.balck_player.id})"
    )
    ...
    if game.game_metadata.get("opening_fen"):
        pgn_game.headers["FEN"] = game.game_metadata["opening_fen"]
...
def export_games_to_file(
    ...
    with open(output_file, "w") as f:
        for game in games:
            pgn_game = export_game_to_pgn(game)
            if pgn_game:
